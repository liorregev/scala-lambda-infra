package com.liorregev.lambda.infra
...
package object serialization {
  ...
  def formatForWithName[T](fieldName: String, formatters: SerializableADT[T]*): OFormat[T] = new OFormat[T] {
    ...
    override def reads(json: JsValue): JsResult[T] =
      read(json).getOrElse {
        JsError(s"Unable to determine which type to use for $fieldName '${(json \ fieldName).asOpt[String].getOrElse("na")}'. Did you forget to manually add it to the list of serializers?")
      }
